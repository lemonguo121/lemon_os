...
class DownloadController extends GetxController {
  ...
  void startDownload(String url) {
    ...
    if (!exists) {
      ...
      downloads.refresh(); // 通知 UI 更新
      SPManager.saveDownloads(downloads); // 保存状态
      print("添加新的下载任务: $url    downloads = ${downloads.length}");
    } else {
  ...
  void updateLocalPath(String url, String path) {
    int index = downloads.indexWhere((d) => d.url == url);
    if (index != -1) {
      downloads[index] = downloads[index].copyWith(localPath: path);
      downloads.refresh();
      SPManager.saveDownloads(downloads);
    }
  }
  ...
  Future<void> _downloadM3u8(String url) async {
    ...
    try {
      ...
      final dir = await _getDownloadDirectory();
      final folder = p.join(dir, 'm3u8_downloads',
      ...
      for (int i = 0; i < segmentUrls.length; i++) {
        ...
        final fullUrl = Uri.parse(segmentUrl).isAbsolute
            ? segmentUrl
            : Uri.parse(url).resolve(segmentUrl).toString();
      ...
      await concatFile.writeAsString(
          localSegments.map((path) => "file '$path'").join('\n'));
